...
class ImageResourceITCase extends ResourceITCase {
    ...
    void doGETAuthorizationWhenAuthorized() {
        Reference uri = getHTTPURI(IIIF3Resource.getURIPath() + "/" +
    ...
    void doGETAuthorizationWhenUnauthorized() {
        Reference uri = getHTTPURI(IIIF3Resource.getURIPath() +
                "/unauthorized.jpg/full/max/0/color.jpg");
    ...
    void doGETAuthorizationWhenForbidden() {
        Reference uri = getHTTPURI(IIIF3Resource.getURIPath() +
                "/forbidden.jpg/full/max/0/color.jpg");
    ...
    void doGETAuthorizationWhenRedirecting() throws Exception {
        Reference uri = getHTTPURI(IIIF3Resource.getURIPath() +
                "/redirect.jpg/full/max/0/color.jpg");
    ...
    void doGETAuthorizationWhenScaleConstraining() throws Exception {
        Reference uri = getHTTPURI(IIIF3Resource.getURIPath() +
                "/reduce.jpg/full/max/0/color.jpg");
    ...
    void doGETCacheHeadersWhenClientCachingIsEnabledAndResponseIsNotCacheable()
        ...
        Reference uri = getHTTPURI(
                IIIF3Resource.getURIPath() + "/bogus/full/max/0/color.jpg");
    ...
    void doGETCacheHeadersWhenClientCachingIsEnabledButCachingIsDisabledInURL1()
            throws Exception {
        Reference uri = getHTTPURI(IIIF3Resource.getURIPath() + "/" +
                ENCODED_IDENTIFIER + "/full/max/0/color.jpg?cache=nocache");
    ...
    void doGETCacheHeadersWhenClientCachingIsEnabledButCachingIsDisabledInURL2()
            throws Exception {
        Reference uri = getHTTPURI(IIIF3Resource.getURIPath() + "/" +
                ENCODED_IDENTIFIER + "/full/max/0/color.jpg?cache=false");
        tester.testCacheHeadersWhenClientCachingIsEnabledButCachingIsDisabledInURL(uri);
    }
    ...
    void doGETCacheHeadersWhenClientCachingIsEnabledAndRecachingIsEnabledInURL()
            throws Exception {
        Reference uri = getHTTPURI(IIIF3Resource.getURIPath() + "/" +
                ENCODED_IDENTIFIER + "/full/max/0/color.jpg?cache=recache");
        tester.testCacheHeadersWhenClientCachingIsEnabledAndResponseIsCacheable(uri);
    }
    ...
    @Test
    void doGETCachingWhenCachesAreEnabledButNegativeCacheQueryArgumentIsSupplied()
            throws Exception {
        Reference uri = getHTTPURI(IIIF3Resource.getURIPath() + "/" +
                ENCODED_IDENTIFIER + "/full/max/0/color.png?cache=false");
    ...
    void doGETCacheWithCachesEnabledAndResolveFirstEnabled() throws Exception {
        ...
        Reference uri = getHTTPURI(IIIF3Resource.getURIPath() + "/" +
                ENCODED_IDENTIFIER + "/full/max/1/color.jpg");
    ...
    void doGETContentDispositionHeaderSetToInline() throws Exception {
        Reference uri = getHTTPURI(IIIF3Resource.getURIPath() + "/" +
                ENCODED_IDENTIFIER +
                "/full/max/0/color.jpg?response-content-disposition=inline");
    ...
    void doGETContentDispositionHeaderSetToAttachment() throws Exception {
        Reference uri = getHTTPURI(IIIF3Resource.getURIPath() + "/" +
                ENCODED_IDENTIFIER + 
                "/full/max/0/color.jpg?response-content-disposition=attachment");
    ...
    void doGETContentDispositionHeaderSetToAttachmentWithFilename()
        ...
        Reference uri = getHTTPURI(IIIF3Resource.getURIPath() + "/" +
                ENCODED_IDENTIFIER + 
                "/full/max/0/color.jpg?response-content-disposition=attachment;filename%3D%22" + filename + "%22");
        tester.testContentDispositionHeaderSetToAttachmentWithFilename(uri, expected);

        uri = getHTTPURI(IIIF3Resource.getURIPath() + "/" + ENCODED_IDENTIFIER +
                "/full/max/0/color.jpg?response-content-disposition=attachment;%20filename%3D%22" + filename + "%22");
        tester.testContentDispositionHeaderSetToAttachmentWithFilename(uri, expected);

        uri = getHTTPURI(IIIF3Resource.getURIPath() + "/" + ENCODED_IDENTIFIER +
                "/full/max/0/color.jpg?response-content-disposition=attachment;filename%3D" + filename);
        tester.testContentDispositionHeaderSetToAttachmentWithFilename(uri, expected);

        uri = getHTTPURI(IIIF3Resource.getURIPath() + "/" + ENCODED_IDENTIFIER +
                "/full/max/0/color.jpg?response-content-disposition=attachment;%20filename%3D" + filename);
        tester.testContentDispositionHeaderSetToAttachmentWithFilename(uri, expected);
    }
    ...
    void doGETEndpointDisabled() {
        ...
        assertStatus(404, getHTTPURI(IIIF3Resource.getURIPath() + "/" +
                ENCODED_IDENTIFIER + "/full/max/0/color.jpg"));
    }
    ...
    void doGETHTTPS1_1() throws Exception {
        Reference uri = getHTTPSURI(IIIF3Resource.getURIPath() + "/" +
                ENCODED_IDENTIFIER + "/full/max/0/color.jpg");
    ...
    void doGETLinkHeaderWithSlashSubstitution() throws Exception {
        ...
        client = newClient(IIIF3Resource.getURIPath() +
                "/sample-imagesCATSjpgCATSjpg.jpg/full/max/0/color.jpg");
        try (Response response = client.send()) {
            String value = response.getHeaders().getFirstValue("Link");
    ...
    void doGETLinkHeaderWithEncodedCharacters() throws Exception {
        ...
        client = newClient(IIIF3Resource.getURIPath() +
                "/sample-images%60jpg%60jpg.jpg/full/max/0/color.jpg");
        try (Response response = client.send()) {
            String value = response.getHeaders().getFirstValue("Link");
    ...
    void doGETLinkHeaderRespectsEndpointPathConfigKey() throws Exception {
        Configuration config = Configuration.forApplication();
        config.setProperty(Key.IIIF_3_ENDPOINT_PATH, "/base");
    ...
    void doGETLinkHeaderRespectsXForwardedHeaders() throws Exception {
        ...
        try (Response response = client.send()) {
            assertEquals("<http://example.org:8080/cats/iiif/3/" +
                            ENCODED_IDENTIFIER +
                            "/32,28,32,28/40,35/0/color.jpg>;rel=\"canonical\"",
                    response.getHeaders().getFirstValue("Link"));
        }
    }
    ...
    void doGETLinkHeaderBaseURIOverridesXForwardedHeaders() throws Exception {
        ...
        try (Response response = client.send()) {
            assertEquals("<https://example.net/iiif/3/" + ENCODED_IDENTIFIER +
                            "/full/max/0/color.jpg>;rel=\"canonical\"",
                    response.getHeaders().getFirstValue("Link"));
        }
    }

    @Test
    void doGETLessThanOrEqualToMaxScale() {
        Reference uri = getHTTPURI(IIIF3Resource.getURIPath() + "/" +
    ...
    void doGETMinPixels() {
        Reference uri = getHTTPURI(IIIF3Resource.getURIPath() + "/" +
                ENCODED_IDENTIFIER + "/0,0,0,0/max/0/color.png");
    ...
    void doGETForbidden() {
        Reference uri = getHTTPURI(
                IIIF3Resource.getURIPath() + "/forbidden.jpg/full/max/0/color.jpg");
    ...
    void doGETNotFound() {
        Reference uri = getHTTPURI(
                IIIF3Resource.getURIPath() + "/invalid/full/max/0/color.jpg");
    ...
    void doGETWithPageNumberInMetaIdentifier() {
        ...
        Reference uri1 = getHTTPURI(
                IIIF3Resource.getURIPath() + "/" + identifier + "/full/max/0/color.jpg");
        Reference uri2 = getHTTPURI(
                IIIF3Resource.getURIPath() + "/" + identifier + ";2/full/max/0/color.jpg");
    ...
    void doGETWithInvalidPageNumber() {
        Reference uri = getHTTPURI(IIIF3Resource.getURIPath() +
                "/sample-images%2Ftif%2Fmultipage.tif;999999/full/max/0/color.jpg");
    ...
    void doGETPurgeFromCacheWhenSourceIsMissingAndOptionIsFalse()
        ...
        String imagePath     = "/" + FILENAME + "/full/max/0/color.jpg";
        OperationList opList = Parameters.fromURI(imagePath)
                .toOperationList(delegate, 1);
        Reference uri        = getHTTPURI(IIIF3Resource.getURIPath() + imagePath);
    ...
    void doGETRecoveryFromIncorrectSourceFormat() throws Exception {
        Reference uri = getHTTPURI(IIIF3Resource.getURIPath() +
                "/sample-images%2Fjpg-incorrect-extension.png/full/max/0/color.jpg");
    ...
    void doGETRedirectToNormalizedScaleConstraint1() {
        ...
        Reference fromURI = getHTTPURI(IIIF3Resource.getURIPath() + "/" +
                metaIdentifierString + "/full/max/0/color.png");
        Reference toURI   = getHTTPURI(IIIF3Resource.getURIPath() + "/" +
                ENCODED_IDENTIFIER + "/full/max/0/color.png");
    ...
    void doGETScaleConstraintIsRespected() throws Exception {
        Reference uri = getHTTPURI(IIIF3Resource.getURIPath() + "/" +
                ENCODED_IDENTIFIER + ";1:2/full/max/0/color.jpg");
    ...
    void doGETRestrictedToSizes() {
        ...
        assertStatus(403, getHTTPURI(IIIF3Resource.getURIPath() + "/" +
                ENCODED_IDENTIFIER + "/full/53,37/0/color.jpg"));
    }
    ...
    void doGETUnavailableSourceFormat() {
        Reference uri = getHTTPURI(
                IIIF3Resource.getURIPath() + "/text.txt/full/max/0/color.jpg");
    ...
    void doGETInvalidOutputFormat() {
        Reference uri = getHTTPURI(IIIF3Resource.getURIPath() + "/" +
                ENCODED_IDENTIFIER + "/full/max/0/color.bogus");
    ...
    void doGETUnsupportedOutputFormat() {
        Reference uri = getHTTPURI(IIIF3Resource.getURIPath() + "/" +
                ENCODED_IDENTIFIER + "/full/max/0/color.pdf");
