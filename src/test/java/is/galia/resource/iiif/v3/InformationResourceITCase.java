...
class InformationResourceITCase extends ResourceITCase {
    ...
    void doGETAuthorizationWhenScaleConstraining() throws Exception {
        Reference uri = getHTTPURI(InformationResource.getURIPath() + "/reduce.jpg/info.json");
    ...
    void doGETCacheHeadersWhenClientCachingIsEnabledAndResponseIsCacheable()
            throws Exception {
        Reference uri = getHTTPURI(
                InformationResource.getURIPath() + "/" + ENCODED_IDENTIFIER + "/info.json");
     ...
     */
    @Test
    void doGETCacheHeadersWhenClientCachingIsEnabledButCachingIsDisabledInURL()
            throws Exception {
        Reference uri = getHTTPURI(InformationResource.getURIPath() + "/" +
    ...
    @Test
    void doGETNotFound() {
        Reference uri = getHTTPURI(InformationResource.getURIPath() + "/invalid/info.json");
    ...
    void doGETURIsInJSON() throws Exception {
        ...
        try (Response response = client.send()) {
            ...
            assertEquals("http://localhost:" + getHTTPPort() +
                    InformationResource.getURIPath() + "/" +
                    ENCODED_IDENTIFIER, info.get("id"));
        }
    }
    ...
    void doGETURIsInJSONRespectEndpointPathConfigKey() throws Exception {
        ...
        Configuration.forApplication()
                .setProperty(Key.IIIF_3_ENDPOINT_PATH, endpointPath);

        String path = IIIF3Resource.getURIPath() + "/" + ENCODED_IDENTIFIER +
                "/info.json";
        ...
        try (Response response = client.send()) {
            ...
            String uri             = client.getURI().rebuilder()
                    .withPath(IIIF3Resource.getURIPath())
    ...
    void doGETURIsInJSONRespectBaseURIConfigKey() throws Exception {
        ...
        try (Response response = client.send()) {
            ...
            ObjectMapper mapper = new ObjectMapper();
            Map<?, ?> info = mapper.readValue(json, Map.class);
            assertEquals("http://example.org" + InformationResource.getURIPath() +
                    "/" + ENCODED_IDENTIFIER,
                    info.get("id"));
        }
    }
