 ...
 *                 <ol>
 *                     <li>A series of ranged {@literal GET} requests (see
 *                     {@link HTTPImageInputStream})</li>
 *                 </ol>
 *             </li>
 *             <li>Else if chunking is disabled:
 *                 <ol>
...
class HTTPSource extends AbstractSource implements Source {
    ...
    private static class ResourceInfo {
        ...
        /**
         * Response entity with a maximum length of {@link
         * FormatDetector#RECOMMENDED_READ_LENGTH}.
         */
        private byte[] entity;
        ...
        Format detectFormat() throws IOException {
            ...
            if (entity != null) {
                try (ImageInputStream is = new ByteArrayImageInputStream(entity)) {
                    format = FormatDetector.detect(is);
                }
            }
            return format;
        }
    ...
    private static Duration getRequestTimeout() {
        int timeout = Configuration.forApplication().getInt(
    ...
    private ResourceInfo fetchResourceInfoViaGET() throws Exception {
        requestInfo = getRequestInfo();
        int length = FormatDetector.RECOMMENDED_READ_LENGTH;
        var extraHeaders = Map.of("Range", "bytes=0-" + (length - 1));
    ...
    HTTPRequestInfo getRequestInfo() throws Exception {
        if (requestInfo == null) {
            ...
            if (LookupStrategy.DELEGATE_SCRIPT.equals(strategy)) {
                requestInfo = newRequestInfoUsingDelegateStrategy();
            } else {
     ...
     */
    private HTTPRequestInfo newRequestInfoUsingDelegateStrategy()
            throws NoSuchFileException, DelegateException {
        final Delegate delegate     = getDelegate();
        final Map<String, ?> result = delegate.getHTTPSourceResourceInfo();
        if (result.isEmpty()) {
